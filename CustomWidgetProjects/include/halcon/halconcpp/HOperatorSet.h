...
LIntExport void ControlIoChannel(const HTuple& IOChannelHandle, const HTuple& Action, const HTuple& Argument, HTuple* Result);
...
LIntExport void CreateDlLayerPermutation(const HTuple& DLLayerInput, const HTuple& LayerName, const HTuple& Permutation, const HTuple& GenParamName, const HTuple& GenParamValue, HTuple* DLLayerPermutation);

// Transform a 3D point with a unit dual quaternion.
LIntExport void DualQuatTransPoint3d(const HTuple& DualQuaternion, const HTuple& Px, const HTuple& Py, const HTuple& Pz, HTuple* Tx, HTuple* Ty, HTuple* Tz);

// Histogram linearization within a rectangluar mask.
LIntExport void EquHistoImageRect(const HObject& Image, HObject* ImageEquHisto, const HTuple& Mode, const HTuple& MaskWidth, const HTuple& MaskHeight, const HTuple& MaxContrast);

// Return the parameters and properties of a measure object.
LIntExport void GetMeasureParam(const HTuple& MeasureHandle, const HTuple& GenParamName, HTuple* GenParamValue);

// Smooth image using a mean filter with arbitrary mask.
LIntExport void MeanImageShape(const HObject& Image, const HObject& Mask, HObject* ImageMean);

// Join strings using separator symbol(s).
LIntExport void TupleJoin(const HTuple& Strings, const HTuple& Separators, HTuple* JoinedStrings);
